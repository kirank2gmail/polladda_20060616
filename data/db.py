...
def verify_password(user_id: str, password: str) -> bool:
    """Verify password — supports both bcrypt (new) and SHA-256 (legacy)."""
    import bcrypt
    u = get_user_by_id(user_id)
    if not u:
        return False
    ...
    # Legacy SHA-256 — valid but will force password reset on next login
    return h == _hash(password)

def is_legacy_password(user_id: str) -> bool:
    """Returns True if user still has a SHA-256 (legacy) password hash."""
    u = get_user_by_id(user_id)
    if not u:
        return False
    return not _is_bcrypt(u.get("password_hash", ""))

def change_password(user_id: str, new_password: str):
    """Always stores password as bcrypt hash going forward."""
